DECLARE @Input VARCHAR(MAX) = '3093	749	3469	142	2049	3537	1596	3035	2424	3982	3290	125	249	131	118	3138
...
-- Part 1
     /* Split the string by newlines for rows (r), then tabs for columns (c) */
WITH ParseTable 
     AS (SELECT RowID = r.ID
              , ColID = c.ID
              , [Number] = CONVERT(INT, c.String)
         FROM Utility.Tools.SplitString(@Input, CHAR(13)+CHAR(10)) r
              CROSS APPLY Utility.Tools.SplitString(r.String, CHAR(9)) c),
     /* For each row, get the difference betweent the max and min values */
     RowCheckSums 
     AS (SELECT RowID
...
WITH ParseTable 
     AS (SELECT RowID = r.ID
         ...
         FROM Utility.Tools.SplitString(@Input, CHAR(13)+CHAR(10)) r
              CROSS APPLY Utility.Tools.SplitString(r.String, CHAR(9)) c),
     /* For each row, find the pair of numbers in each row where the smaller evenly divides into the larger (using modulus here) */
     RowCheckSums 
     AS (SELECT pt1.RowID
              , [CheckSum] = pt1.Number/pt2.Number
         FROM ParseTable pt1
              JOIN ParseTable pt2
                ON pt1.RowID = pt2.RowID
                   AND pt1.Number % pt2.Number = 0
                       AND pt1.Number > pt2.Number)
     SELECT SUM(CheckSum)
     FROM RowCheckSums;