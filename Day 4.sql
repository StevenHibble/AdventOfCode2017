DECLARE @Input VARCHAR(MAX) = 'vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
-- Part 1
     -- Split the input into separate passphrases and then words
WITH Words
     AS (SELECT PassphraseID = r.ID
              , WordID = w.ID
              , Word = w.String
         FROM Utility.Tools.SplitStringMAX(@Input, CHAR(13)+CHAR(10)) r
              CROSS APPLY Utility.Tools.SplitStringMAX(r.String, ' ') w)
     -- Filter out passphrases with duplicate words
   , ValidPassPhrases
     AS (SELECT PassphraseID
         FROM Words
         GROUP BY PassphraseID
         HAVING COUNT(Word) = COUNT(DISTINCT Word))

SELECT Solution1 = COUNT(*)
FROM ValidPassPhrases;

     -- Split the input into separate passphrases, then words, then letters
WITH Letters
     AS (SELECT PassphraseID = r.ID
              , WordID = w.ID
              , LetterID = l.ID
         ...
         FROM Utility.Tools.SplitStringMAX(@Input, CHAR(13)+CHAR(10)) r
              CROSS APPLY Utility.Tools.SplitStringMAX(r.String, ' ') w
              CROSS APPLY Utility.Tools.SplitStringMAX(w.String, '') l)
     -- Rearrange each word in letter order (producing equal words for anagrams)
     -- Using SQL Server 2017 STRING_AGG... because otherwise I would've had to write a gross XML-based function here
   , RearrangedWords
     AS (SELECT PassphraseID
              , WordID
              , Word = STRING_AGG(Letter, '') WITHIN GROUP (ORDER BY Letter)
         FROM Letters
         GROUP BY PassphraseID
                , WordID)
     -- Filter out passphrases with duplicate anagrams
   , ValidPassPhrases
     AS (SELECT PassphraseID
         FROM RearrangedWords
         GROUP BY PassphraseID
         HAVING COUNT(Word) = COUNT(DISTINCT Word))

SELECT Solution2 = COUNT(*)
FROM ValidPassPhrases;