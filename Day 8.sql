DECLARE @Input VARCHAR(MAX) = 'q inc -541 if c != 4
s inc -555 if o > -5
px dec -84 if x >= -4
y dec -822 if txk > -1
...
DROP TABLE IF EXISTS #Instructions, #Registers;
SELECT instr.ID
     , Register = CONVERT(VARCHAR(8), MAX(IIF(units.ID = 1, units.String, NULL)))
     , Increment = IIF(MAX(IIF(units.ID = 2, units.String, NULL)) = 'dec', -1, 1) * MAX(IIF(units.ID = 3, CONVERT(INT, units.String), NULL))
     , ConditionRegister = CONVERT(VARCHAR(8), MAX(IIF(units.ID = 5, units.String, NULL)))
     , Condition = CONVERT(VARCHAR(MAX), REPLACE(MAX(IIF(units.ID = 6, units.String, NULL)), '==', '=') + ' ' + MAX(IIF(units.ID = 7, units.String, NULL)))
INTO #Instructions
FROM Manage.Tools.SplitStringMAX(@Input, CHAR(13)+CHAR(10)) instr
     CROSS APPLY Manage.Tools.SplitStringMAX(instr.String, ' ') units
GROUP BY instr.ID;
...
CREATE UNIQUE CLUSTERED INDEX CIX ON #Registers (Register);

DECLARE @DynamicSQL VARCHAR(MAX)
DECLARE @MaxValue INT = 0;
DECLARE @Counter INT = 1;
DECLARE @Total INT = (SELECT MAX(ID) FROM #Instructions)
...
WHILE @Counter <= @Total
  BEGIN
    ...
    SELECT @DynamicSQL = 
           CONCAT('UPDATE r1 
                   SET [Value] = r1.[Value] + (', Increment, ')
                   FROM #Registers r1,
                        #Registers r2
                   WHERE r1.Register = ''', Register, ''' 
                         AND r2.Register = ''', ConditionRegister, '''
                         AND r2.[Value] ', Condition, ';')
    FROM #Instructions
    WHERE ID = @Counter;
    
    EXEC(@DynamicSQL);

    SELECT @MaxValue = IIF(MAX([Value]) > @MaxValue, MAX([Value]), @MaxValue)
    FROM #Registers;

    SET @Counter += 1;
  END;

SELECT Solution1 = MAX([Value])
FROM #Registers

SELECT Solution2 = @MaxValue;